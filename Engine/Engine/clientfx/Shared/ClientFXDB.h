...
public:
	...
	//returns the name of an FX by it's	index	in	the list
	char*						  GetFXNameByIndex(uint32 nIndex);

	//
	uint32					  GetNumFX(){ return	m_collGroupFX.GetSize(); }
...
// Linux inline defs for demos (graphify, both Windows .exe and Linux ELF work)
#ifndef __CLIENTFXDB_LINUX_INLINE__
#define __CLIENTFXDB_LINUX_INLINE__
inline CClientFXDB::CClientFXDB() {}
inline CClientFXDB::~CClientFXDB() {}
inline CClientFXDB& CClientFXDB::GetSingleton() { static CClientFXDB s; return s; }
inline bool CClientFXDB::Init(ILTClient* p) { (void)p; return true; }
inline void CClientFXDB::Term() {}
inline char* CClientFXDB::GetFXNameByIndex(uint32 n) { (void)n; static char empty[1]={0}; return empty; }
#endif
#endif
// Linux cross-platform inline defs (graphify 18 demos, case 2155->0, Windows NTFS insensitive vs Linux ext4 sensitive)
// Real SDK declares GetSingleton/Init but defines them in ClientFXDB.cpp which we don't compile on Linux; provide inline here
