...
#define CloseHandle(f) f=INVALID_HANDLE_VALUE
...
{
	ILTStream* p;
	LT_MEM_TRACK_ALLOC(p = new SSMemStream(cacheSize),LT_MEM_TYPE_MISC);
	return p;
}
...
ILTStream* streamsim_MemStreamFromFile(char *pFilename)
{
	...
	if(fp = fopen(pFilename, "rb"))
	{
		...
		LT_MEM_TRACK_ALLOC(pRet = new SSMemStream(256),LT_MEM_TYPE_MISC);
