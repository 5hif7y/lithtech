...
public:
	...
	LTRESULT	Read(void *pData, uint32 size)
	{
		...
		{
			if( ReadFile( m_hFile, pData, size, (unsigned long*)&dwBytesRead, LTNULL ) && dwBytesRead == size )
			{
				return LT_OK;
			}
			else
	...
	LTRESULT Write( const void *pData, uint32 size )
	{
		...
		{
			if( WriteFile( m_hFile, pData, size, (unsigned long*)&dwBytesWritten, LTNULL ) && dwBytesWritten == size )
			{
				return LT_OK;
			}
			else
...
{
	ILTStream* p;
	LT_MEM_TRACK_ALLOC(p = new SSMemStream(cacheSize),LT_MEM_TYPE_FILE);
	return p;
}
...
{
	ILTStream* p;
	LT_MEM_TRACK_ALLOC(p = new SSAbstractIOWrapper(pIO),LT_MEM_TYPE_FILE);
	return p;
}
...
{
	SSBufStream *pRet;
	LT_MEM_TRACK_ALLOC(pRet = new SSBufStream( pData, dwDataSize ),LT_MEM_TYPE_FILE);
	return pRet;
}
