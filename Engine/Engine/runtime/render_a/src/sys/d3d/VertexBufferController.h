...
public:
	...
	bool							CreateStream(uint32 iStreamNum, int32 nVertexCount, uint32 iVertDataType, VERTEX_BLEND_TYPE VertBlendType, bool bDynamic, bool bWriteOnly, bool bForceSWVertProc, bool bUsagePoints = false);
	bool							CreateIndexBuffer(int32 iIndexCount, bool bDynamic, bool bWriteOnly, bool bForceSWVertProc);
	...
	void							Render(uint32 MinIndex,uint32 MinVert,uint32 VertCount,uint32 TriCount, uint32 iVertOffset = 0);	// Transform, light, and render directly using the unprocessed buffer...
	void							RenderPoints(uint32 StartVert,uint32 PointCount);	// For rendering non-indexed points...

	bool							Lock(VB_TYPE Type,bool bDiscardContents,bool bNoOverWrite=false,uint32 iOffsetToLock_Bytes = 0,uint32 iSizeToLock_Bytes = 0);
...
private:
	LPDIRECT3DVERTEXBUFFER9			m_pVB_VertStream[VERTEXBUFFER_STREAMCOUNT];	// Handles to the vertex buffers...
	LPDIRECT3DINDEXBUFFER9			m_pVB_Index;
