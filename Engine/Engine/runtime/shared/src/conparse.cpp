...
// [KLS] The normal iscntrl in some cases incorrectly returns true for -127 in release 
// builds.  For multi-byte languages (e.g., Japanese) this is a valid non-control value.
// So, Parse_Iscntrl should be used to insure that parsing works correctly for multi-byte
// languages.
 
static LTBOOL Parse_Iscntrl(char c)
{
	if ((0 <= c && c <= 31) || c == 127) return LTTRUE;
...
inline LTBOOL cp_IsCmdSeparator(char curChar)
{
	return curChar == ';' || Parse_Iscntrl(curChar);
}
