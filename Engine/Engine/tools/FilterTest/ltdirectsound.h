...
typedef enum
{
   ...
   RIB_DATA_SUBTYPE_PACK = 0xFFFFFFFF
}
RIB_DATA_SUBTYPE;
...
typedef enum
{
   ...
   RIB_ENTRY_TYPE_PACK = 0xFFFFFFFF // Hard-coded entry to force a 32-bit value (MSS SUCKS!!)
}
RIB_ENTRY_TYPE;
...
typedef void (AILCALLBACK FAR* AILINCB)       (void FAR *data, S32 len, U32 user_data);
...
DXDEC char FAR*     AILCALL AIL_set_redist_directory(char  FAR*dir);
