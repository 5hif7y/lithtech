...
//----------------------------------------------------------
	...
	{				
		public:
			...
			BOOL					AddHead(T data);
			BOOL					AddTail(T data);
			CLinkListNode<T>*		InsertAfter(CLinkListNode<T> *pNode, T data);
			CLinkListNode<T>*		InsertBefore(CLinkListNode<T> *pNode, T data);
			T						RemoveHead();
			T						RemoveTail();
			...
			T						Get(DWORD dwIndex);
			int						GetIndex(T data);
			void					CopyList(CLinkList<T> *pList);
			void					WriteList(FILE *fp);
			void					ReadList(FILE *fp);
	...
	template <class T> inline CLinkListNode<T>* CLinkList<T>::InsertAfter(CLinkListNode<T> *pNode, T data)
	{
		CLinkListNode<T> *pNewNode = new CLinkListNode<T>;
	...
	template <class T> inline CLinkListNode<T>* CLinkList<T>::InsertBefore(CLinkListNode<T> *pNode, T data)
	{
		CLinkListNode<T> *pNewNode = new CLinkListNode<T>;
		if (!pNewNode) return NULL;
	...
	template <class T> inline T CLinkList<T>::Get(DWORD dwIndex)
	{
		CLinkListNode<T> *pNode = m_pHead;
	...
	//----------------------------------------------------------
	//
	// FUNCTION : CLinkList::Copy()
	//
	// PURPOSE	: Copies a list into this list
	//
	//----------------------------------------------------------

	template <class T> inline void CLinkList<T>::CopyList(CLinkList<T> *pList)
	{
		RemoveAll();

		CLinkListNode<T> *pNode = pList->GetHead();

		while (pNode)
		{
			AddTail(pNode->m_Data);
	...
	//----------------------------------------------------------
	//
	// FUNCTION : CLinkList::WriteList()
	//
	// PURPOSE	: Writes a list out to a file stream
	//
	//----------------------------------------------------------

	template <class T> inline void CLinkList<T>::WriteList(FILE *fp)
	{
		// Write out the number of elements in this list

		fwrite(&m_nSize, sizeof(DWORD), 1, fp);

		// Write out the actual elements

		CLinkListNode<T> *pNode = GetHead();

		while (pNode)
		{
			fwrite(&pNode->m_Data, sizeof(T), 1, fp);
	...
	//----------------------------------------------------------
	//
	// FUNCTION : CLinkList::ReadList()
	//
	// PURPOSE	: Writes a list out to a file stream
	//
	//----------------------------------------------------------

	template <class T> inline void CLinkList<T>::ReadList(FILE *fp)
	{
		DWORD nSize;
