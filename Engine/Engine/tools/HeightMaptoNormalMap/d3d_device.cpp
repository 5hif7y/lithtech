...
void CD3D_Device::TryFallingBack_OnFailedDevCreate(D3DPRESENT_PARAMETERS* pPresentationParam)
{
	...
	if (pPresentationParam->Windowed) {
	}
	else { // FullScreen...
	}
}
...
{
	if (m_pD3DDevice) {
		uint32 iRefCnt = m_pD3DDevice->Release(); } // assert(iRefCnt==0);
...
D3DMULTISAMPLE_TYPE CD3D_Device::GetDefaultMultiSampleType(uint32 Samples)
{
	...
	if ( PDIRECT3D->CheckDeviceMultiSampleType(m_pAdapter->iAdapterNum,m_pDevice->DeviceType,m_pMode->Format,g_bRunWindowed,MultiSampleType, NULL) != D3D_OK)
	{
		...
		{
			return GetDefaultMultiSampleType((uint32)MultiSampleType-1); 
		}
		else
		{
			return (D3DMULTISAMPLE_TYPE)0; 
		}
	}
...
//		SurfDesc.Size   = sizeof(SurfDesc);
		...
		m_ExtraDevCaps.m_DepthStencilBufferFormat = SurfDesc.Format; 
		
		uint32 iZDepth,iStencilDepth;
...
//	m_pD3DDevice->SetRenderState(D3DRS_EDGEANTIALIAS, FALSE); // default line antialias is FALSE
...
//	default : return NULL; } //assert(0);  }
	...
	if (pNewObject) {							// Add it to the Render Object List...
