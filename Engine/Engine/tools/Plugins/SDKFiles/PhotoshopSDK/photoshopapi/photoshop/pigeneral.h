...
//
//-------------------------------------------------------------------------------
// $Id: //photoshop/8.0/photoshop/sharedinterfaces/PIGeneral.h#1 $ 
...
#if MSWindows
	#define PIVersion "8.0"
	#define ProductVersion  8,00,0x06,0
...
// "This plug-in is not compatible with this version of Photoshop."
...
// 'frag' <PIFragmentCodeDesc> PowerPC CARBON fragment descriptor. See struct below.
...
// For 'frag' PICodeFragmentProperty:
// takes precendence over PIPowerPCCodeProperty
typedef struct PIFragmentCodeDesc
{
	...
	long fFragmentNumber;
	
	// Name of entrypoint ("main", "ENTRYPOINT", "", etc.):
	// if blank (""), the first entrypoint in the fragment is used
	unsigned char fEntryName[1];
...
// 'flly' plug in filtery layer properties
#define PIFilterLayerSupport	0x66262679L
...
typedef struct PSImageMultiPlane32
{
	...
	int32 colBytes;
	int16 activeChannels;
	int16 depth;			// only 8 and 16 bit unsigned values are valid
...
#define kCurrentMaxVersReadLayerDesc	0
...
typedef struct ReadLayerDesc
{
	...
	//---------------------------------------------------------------------------
	// The next descriptor in the list:
	struct ReadLayerDesc *next;
	
	ReadChannelDesc *compositeChannelsList;
	
	// Transparency channel for the layer:
	...
	Boolean isPixelBased;	// Is this layer pixel based ?
...
#define kCurrentMaxVersReadImageDocDesc	2
...
typedef struct ReadImageDocumentDesc
{
	...
	// descriptor for layers, a linked list, if any:
	
	ReadLayerDesc *layersDescriptor;
	
	// Document type. See above.
	int32 documentType;
	
	// The next descriptor in the list:
	struct ReadImageDocumentDesc *next;
...
#define PIMaxSize32Property	 'ms32' 	/* <VPoint> Max rows and cols supported by this plugin. 	*/
#define PIMinSize32Property  'mn32'     /* <VPoint> Min rows and cols supported by this plugin.     */
