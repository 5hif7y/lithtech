...
//-------------------------------------------------------------------------------
char* gActionName = NULL;
char* gActionSet = NULL;
...
)
{
	...
	if (data != NULL)
	{
		...
		if (listener != NULL)
			SPErr error = PIUActionsPlayByName(listener->actionSet, 
				                           	   listener->actionName);
	}
}
...
)
{
	...
	SPErr error = GetFullPathToDesktop(logfilename);
	if (error)
		strcpy(logfilename, "");
...
static SPErr Shutdown (void)
{
	...
	while (thisList != NULL)
	{
		...
		gActionName = thisList->actionName;
		gActionSet = thisList->actionSet;
		
		if (gActionName != NULL)
		{
			delete [] gActionName;
			gActionName = NULL;
			thisList->actionName = NULL;
		}
