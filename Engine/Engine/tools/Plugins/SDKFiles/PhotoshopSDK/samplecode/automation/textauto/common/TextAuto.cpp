...
static void OpenEvent( DescriptorEventID event,
...
//
//-------------------------------------------------------------------------------
TextAutoPlugin::TextAutoPlugin(SPPluginRef pluginRef)
  : AutomationPlugin(pluginRef), fNotifierOn(false)
{
}

ASErr TextAutoPlugin::StartupPlugin(SPInterfaceMessage * /* message */)
{
	ASErr error = kSPNoError;
	
	if (!fNotifierOn)
	{
		error = sPSActionControl->AddNotify(pluginRef,
											eventOpen,
											::OpenEvent,
											this);
...
ASErr TextAutoPlugin::ShutdownPlugin(SPInterfaceMessage * /* message */)
{
	ASErr error = 0;

	if (fNotifierOn)
		error = sPSActionControl->RemoveNotify(pluginRef,
											   eventOpen);
	return error;
}
...
//	AllocatePlugin
//	
//	The real constructor hear. You have to have one of these so the	AutoPluginMain 
//	routine can create your class.
//
//-------------------------------------------------------------------------------
AutomationPlugin * AllocatePlugin(SPPluginRef pluginRef)
{
	return new TextAutoPlugin(pluginRef);
}
...
void OpenEvent( DescriptorEventID event,
				...
				void * data)
{
	TextAutoPlugin * textAutoPlugin = NULL;

	try 
	{
		textAutoPlugin = reinterpret_cast<TextAutoPlugin *>(data);
		
		textAutoPlugin->OpenEvent(event, descriptor, options);
	}
...
void TextAutoPlugin::OpenEvent(DescriptorEventID event,
				               PIActionDescriptor descriptor,
				               PIDialogRecordOptions /* options */)
{
	...
	AliasToFullPath(aliasValue, &fullPathToFile[0]);
...
void TextAutoPlugin::MakeTextLayer(string fullPath, VRect rect)
{
	...
	(void)sPSActionDescriptor->PutInteger(textStyleRangeObject.get(), keyTo, fileContents.str().length() + 1);

	Auto_Desc textStyleObject;

	(void)sPSActionControl->StringIDToTypeID("fontPostScriptName", &runtimeKeyID);
	(void)sPSActionDescriptor->PutString(textStyleObject.get(), runtimeKeyID, "Helvetica");
	(void)sPSActionDescriptor->PutString(textStyleObject.get(), keyFontName, "Helvetica");
	(void)sPSActionDescriptor->PutString(textStyleObject.get(), keyFontStyleName, "Medium");
	...
	(void)sPSActionDescriptor->PutInteger(paragraphRangeObject.get(), keyTo, fileContents.str().length() + 1);
	...
	(void)sPSActionDescriptor->PutString(paragraphStyleObject.get(), runtimeKeyID, "None");
...
void TextAutoPlugin::DeleteBackground(void)
{
	Auto_Desc result(false);
	Auto_Desc desc;
	Auto_Ref ref;
