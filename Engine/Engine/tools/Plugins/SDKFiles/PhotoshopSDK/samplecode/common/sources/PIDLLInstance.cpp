...
//
//-------------------------------------------------------------------------------
const char * const seasonArray[] = { "DLL_PROCESS_DETACH",
"DLL_PROCESS_ATTACH", "DLL_THREAD_ATTACH", "DLL_THREAD_DETACH", "DLL_PROCESS_VERIFIER" };
...
BOOL APIENTRY DllMain(HANDLE hModule,
					  ...
					  LPVOID /*lpReserved*/)
{
	FILE * myFile = fopen("C:\\LoadLogTextFormat.txt", "a+");
	if (myFile)
	{
		time_t ltime;
		time (&ltime);
		fprintf(myFile, "%s DllMain: %s", seasonArray[ul_reason_for_call], ctime(&ltime));
		fclose(myFile);
	}
