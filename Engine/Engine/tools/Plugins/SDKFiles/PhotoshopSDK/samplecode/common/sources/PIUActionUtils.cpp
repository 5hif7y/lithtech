...
static FreeZone* freeZone = NULL;
static int16 maxHashStringSize = 1000;
	...
	while (counter < listCount)
	{
		...
		switch (typeID)
		{
			...
			case typeChar:
				...
				#else
					multiByteLength = WideCharToMultiByte(CP_UTF8, 0, unicodeStr, unicodeLength,
														  NULL, 0, NULL, NULL);
					if (multiByteLength > 0)
					{
						multiByteStr = new char[multiByteLength];
						if ( WideCharToMultiByte(CP_UTF8, 0, unicodeStr, unicodeLength,
			...
			case typeAlias:
				...
				if (fullPath != NULL)
					AliasToFullPath(aliasValue, fullPath);
	...
	if (!found)
	{
	    for (int32 counter = 0; counter < ArraySize; counter++)
    	    if (PSConstA[counter].longVal == inputLong)
				...
				{
					short strLen = strlen(PSConstA[counter].strStr);
...
//
//-------------------------------------------------------------------------------
void FreeZone::Add(char* text, long value)
{
