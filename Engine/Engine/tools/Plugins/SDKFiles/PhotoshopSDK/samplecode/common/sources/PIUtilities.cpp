...
int16 CSModeToSpace (const int16 imageMode)
{
	...
	if (imageMode >= plugInModeBitmap && imageMode <= plugInModeDuotone16)
	{
					 	 ...
					 	 int32 *outSize)
{
	...
	Handle			h = NULL;
	Ptr				p = NULL, p1 = NULL; // LPVOID Ptr hPtr;
	char			name[5] = "\0\0\0\0";
	...
	*outSize = 0;
	...
	else
	{
		...
		{
			size_t size = 0;
			h1 = LoadResource(GetDLLInstance(), (HRSRC)h2);
