...
//									See PIFilter.h
//
//		int32 *data					Use this to store a handle to our global
...
void DoPrepare(void)
{
	if (*gDataHandle != NULL && gFilterRecord->parameters != NULL)
		LockHandles();
	else
	{
		if (gFilterRecord->parameters == NULL)
...
void DoFilter(void)
{
	...
	for (int32 vertTile = 0; vertTile < tilesVert; vertTile++)
	{
		for (int32 horizTile = 0; horizTile < tilesHoriz; horizTile++)
		{
			...
			if (gFilterRecord->haveMask != NULL)
			{
				SetMaskRect(inRect);
			}
...
// because it handles both 8 bit and 16 bit images.
//
//-------------------------------------------------------------------------------
void DissolveRectangle(void* data, 
					   int32 dataRowBytes, 
...
void LockHandles(void)
{
	if (gFilterRecord->parameters == NULL || *gDataHandle == NULL)
	{
		*gResult = filterBadParameters;
		return;
	}
