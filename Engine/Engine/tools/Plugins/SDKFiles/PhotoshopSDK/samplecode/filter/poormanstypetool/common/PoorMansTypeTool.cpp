						             ...
						             short *result)
{
	...
	{ // do the rest of the process as normal:
		...
		globalPtr = AllocateGlobals ((uint32)result,
									 ...
									 ((FilterRecordPtr)filterParamBlock)->handleProcs,
									 sizeof(Globals),
						 			 data,
						 			 InitGlobals);
		...
		// called.
		if (gPSChannelPortsSuite == NULL || gPSBufferSuite == NULL)
		{
...
void DoEffect(GPtr globals)
{
	...
	char *pLayerData = gPSBufferSuite->New(NULL, 
	...
	gPSBufferSuite->Dispose((char**)&pLayerData);
}
...
void GaussianBlurEffect(GPtr globals, Rect *inRect, Rect *containmentRect, char* data)
{
	...
	char *localData = gPSBufferSuite->New(NULL, vSize * hSize);
	if (localData == NULL)
		return;
	...
	gPSBufferSuite->Dispose((char**)&originalLocal);
}
...
void UpdateProxyView(GPtr globals)
{
	...
	{
		gChannelData = gPSBufferSuite->New(NULL, memorySize);
		if (gChannelData == NULL)
		{
			gResult = memFullErr;
			return;
	...
	if (pChannel != NULL)
	{
		if (gMaskData == NULL)
		{
			gMaskData = gPSBufferSuite->New(NULL, memorySize);
			if (gMaskData == NULL)
			{
				gResult = memFullErr;
				return;
	...
	if (gDocDesc->selection != NULL)
	{
		...
		{
			gSelectionData = gPSBufferSuite->New(NULL, memorySize);
			if (gSelectionData == NULL)
			{
				gResult = memFullErr;
				return;
	...
	{
		gOverlayData = gPSBufferSuite->New(NULL, memorySize);
		if (gOverlayData == NULL)
		{
			gResult = memFullErr;
			return;
...
void ReleaseProxyMemory(GPtr globals)
{
	if (gSelectionData != NULL)
	{
		gPSBufferSuite->Dispose((char**)&gSelectionData);
		gSelectionData = NULL;
	}
	
	if (gMaskData != NULL)
	{
		gPSBufferSuite->Dispose((char**)&gMaskData);
		gMaskData = NULL;
	}
	
	if (gChannelData != NULL)
	{
		gPSBufferSuite->Dispose((char**)&gChannelData);
		gChannelData = NULL;
	}
	
	if (gOverlayData != NULL)
	{
		gPSBufferSuite->Dispose((char**)&gOverlayData);
		gOverlayData = NULL;
	}
}
/****************************************************************************/
