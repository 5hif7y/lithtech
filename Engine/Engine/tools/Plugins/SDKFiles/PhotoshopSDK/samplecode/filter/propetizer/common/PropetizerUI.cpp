...
int32 PropetizerPlugin::DoUI(void)
{
...
void PropetizerDialog::Notify(int32 index)
{
	...
	if (index == 1)
	{
		...
		textPointer = item.GetText();
		if (textPointer != NULL)
		{
			SetBigNudgeV(atof(textPointer));
			delete [] textPointer;
		}
		...
		if (textPointer != NULL)
		{
			SetRulerOriginV(atof(textPointer));
			delete [] textPointer;
		}
		...
		if (textPointer != NULL)
		{
			SetRulerOriginH(atof(textPointer));
			delete [] textPointer;
		}
		...
		if (textPointer != NULL)
		{
			SetWatchSuspension(atoi(textPointer));
			delete [] textPointer;
		}
		...
		if (textPointer != NULL)
		{
			SetSliceIndex(atoi(textPointer));
			delete [] textPointer;
		}
	}
}
