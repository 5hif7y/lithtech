...
const unsigned short kPrecision = kPIUQuadPrecision; // 4 decimal places.
...
static char* GetUnitsText
	(
	/* IN */	const RulerUnits units
	);
...
static SPErr InitUnitsPopUp(ADMDialogRef dialog)
	...
	short numberOfEntries = sADMList->NumberOfEntries(list);
	...
	for (loop = (short)kRulerPixels; loop <= (short)kRulerPercent; loop++)
		...
		if (entry != NULL)
			{
			char* text = GetUnitsText((RulerUnits)loop);
			sADMEntry->SetText(entry, text);
			if (text != NULL)
				{
				sSPBasic->FreeBlock(text);
				text = NULL;
				}
			}
		}
...
static void SetTextToCancel
	...
	if (item != NULL)
		{
		char* resourceCancelString = PIUGetString(kCancelStringID);
		char* hardCancelString = "Cancel";
		char* cancelString = resourceCancelString;
		
		if (resourceCancelString == NULL)
			{
			// Oops.  Resource unavailable.  Use hard coded.
			cancelString = hardCancelString;
			}
		
		sADMItem->SetText(item, cancelString);
		
		// If resource was loaded, free it:
		if (resourceCancelString != NULL)
			{
			sSPBasic->FreeBlock(resourceCancelString);
			resourceCancelString = NULL;
			}
...
static void SetTextToReset
	...
	if (item != NULL)
		{
		char* resourceResetString = PIUGetString(kResetStringID);
		char* hardResetString = "Reset";
		char* resetString = resourceResetString;
		
		if (resourceResetString == NULL)
			{
			// Oops.  Resource unavailable.  Use hard coded.
			resetString = hardResetString;
			}
		...
		if (resourceResetString != NULL)
			{
			sSPBasic->FreeBlock(resourceResetString);
			resourceResetString = NULL;
			}
...
//	GetUnitsText
//
//	Return a string with the units.  Caller must dispose.
//
//-------------------------------------------------------------------------------
static char* GetUnitsText
	(
	...
	{
	return PIUGetString(kUnitsID+units);
	}
...
static void ASAPI DoUnitsPopUp
	...
	if (sADMNotify->IsNotifierType(notifier, kADMUserChangedNotifier))
		...
		short index = sADMEntry->GetIndex(entry);
