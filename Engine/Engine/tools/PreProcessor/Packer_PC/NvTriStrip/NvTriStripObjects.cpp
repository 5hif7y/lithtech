	...
	{	
		...
		if(!AlreadyExists(faceInfo, faceInfos))
		{
			...
			if (edgeInfo01 == NULL){
				...
				{
					DrawStatusText(eST_Error, "BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences\n");
				}
				else
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
	...
	if (fv0 != v0 && fv0 != v1){
		if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
			DrawStatusText(eST_Error, "GetNextIndex: Triangle doesn't have all of its vertices\n");
			DrawStatusText(eST_Error, "GetNextIndex: Duplicate triangle probably got us derailed\n");
		}
		return fv0;
	}
	if (fv1 != v0 && fv1 != v1){
...
void NvStripifier::RemoveSmallStrips(NvStripInfoVec& allStrips, NvStripInfoVec& allBigStrips, NvFaceInfoVec& faceList)
{
	...
	{
		if(allStrips[i]->m_faces.size() < (unsigned int)minStripLength)
		{
