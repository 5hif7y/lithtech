...
public:
	...
	const char* GetInFile(uint32 i)			{ char szTmp[32]; sprintf(szTmp,"input%d",i+1); return GetParamVal(szTmp); }
	const char* GetOutFile()				{ return GetParamVal("output"); }
	const char* GetTargetPlatform()         { return GetParamVal("target"); }
	...
	bool hasTarget() {
		const char *val= "target";vector<CommandPair>::iterator it=m_CmdLine.begin();while(it!=m_CmdLine.end()){if(stricmp(it->szParam.c_str(),val)==0)return true;++it;}
		return false; }

	int argv() { return m_CmdLine.size() ; } 
	...
	virtual void ParseParam(LPCTSTR szParam, BOOL bFlag, BOOL bLast) {
		if (bFlag && m_bOnCmd) {			// Parsing a command now...
			...
			if (szParam) {
				...
				if (stricmp(szParam, "noautostart") == 0) m_bOnCmd = true;
