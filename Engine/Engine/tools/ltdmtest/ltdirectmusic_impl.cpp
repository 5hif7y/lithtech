		  						     ...
		  						     const char* sDefine2, const char* sDefine3)
{
		...
		(strchr(sControlFileName,':') == LTNULL))
	{
		...
		if (!controlFile.Init(sNewControlFileName)) { LTDMConOutError("ERROR! Unable to read control file. (CLTDirectMusicMgr::InitLevel)\n"); return LT_ERROR; }
	...
	{
		// initialize control file
		if (!controlFile.Init(sControlFileName)) { LTDMConOutError("ERROR! Unable to read control file. (CLTDirectMusicMgr::InitLevel)\n"); return LT_ERROR; }
	}
