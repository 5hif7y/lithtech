...
uint32 CScreenSave::OnCommand(uint32 dwCommand, uint32 dwParam1, uint32 dwParam2)
{
	...
	{
		SAFE_STRCPY(m_szSaveName,(char *)dwParam1);
		ForceMouseUpdate();
...
void CScreenSave::NameSaveGame(uint32 slot, int index)
{
	...
	if (!g_pMissionMgr->IsCustomLevel())
	{
		...
		if (pMission)
		{
			...
			{
				char szScene[64];
				FormatString(IDS_SCENENUMBER, szScene, sizeof(szScene), sceneNum + 1);
				sprintf(m_szSaveName,"%s, %s",szMission,szScene);
			}
		}
	}
