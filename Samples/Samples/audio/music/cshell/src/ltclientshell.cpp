...
LTRESULT CLTClientShell::OnEngineInitialized(RMode *pMode, LTGUID *pAppGuid)
{
	...
	m_Gui.Init();
	m_Gui.ApplyPositionStyle(PS_TOP | PS_LEFT);
	UpdateGuiText(m_nCurIntensity);
	
	// Initialize music playback
	g_pLTCDirectMusicMgr = g_pLTClient->GetDirectMusicMgr();
	result = g_pLTCDirectMusicMgr->Init();
	DebugPrintIfFailed(result, "Error initializing music.\n");
...
void CLTClientShell::OnMessage(ILTMessage_Read* pMessage)
{
	...
	case MSG_TRG_PLAYMUSIC:
		{
			// Get the new intesity from the trigger object
			int newInt = pMessage->Readint32();

			// Set and play the new intensity
			m_nCurIntensity = newInt;
			g_pLTCDirectMusicMgr->Play();
			g_pLTCDirectMusicMgr->ChangeIntensity(m_nCurIntensity, LTDMEnactImmediately);
			UpdateGuiText(m_nCurIntensity);
		}
		break;
...
void CLTClientShell::Update()
{
	...
	if( m_bFirstUpdate )
	{
		...
		// Init our direct music level info
		LTRESULT result = g_pLTCDirectMusicMgr->InitLevel("\\Music","music.txt");
		DebugPrintIfFailed(result, "Error in loading direct music level info");
...
void CLTClientShell::OnEvent(uint32 dwEventID, uint32 dwParam)
{
	...
	if (dwEventID == LTEVENT_RENDERTERM)
	{
        m_bRender = false;
		...
		case COMMAND_PLAY_MUSIC:
			{
				...
				if(m_bMusicPaused)
				{
					...
					g_pLTCDirectMusicMgr->UnPause();
					UpdateGuiText(m_nCurIntensity);
				}
				else
				{
					if(g_pLTCDirectMusicMgr->GetCurIntensity() != m_nCurIntensity)
					{
						g_pLTCDirectMusicMgr->Play();
						g_pLTCDirectMusicMgr->ChangeIntensity(m_nCurIntensity, LTDMEnactImmediately);
		...
		case COMMAND_STOP_MUSIC:
			{
				...
				g_pLTCDirectMusicMgr->Stop(LTDMEnactImmediately);
			}
			break;
		case COMMAND_TOGGLE_MUSIC_PAUSE:
			{
				m_bMusicPaused = !m_bMusicPaused;

				if(m_bMusicPaused)
				{
					g_pLTCDirectMusicMgr->Pause();
				}
				else
				{
					g_pLTCDirectMusicMgr->UnPause();
			...
			break;
		case COMMAND_NEXT_INTENSITY:
			{
				if(m_nCurIntensity < LAST_INTENSITY)
				{
					++m_nCurIntensity;
					UpdateGuiText(m_nCurIntensity);
				}
			...
			break;
		case COMMAND_PREV_INTENSITY:
			{
				if(m_nCurIntensity > 0)
				{
					--m_nCurIntensity;
					UpdateGuiText(m_nCurIntensity);
				}
				...
				{
					m_nCurIntensity = LAST_INTENSITY;
					UpdateGuiText(m_nCurIntensity);
				}
			}
			break;	
		case COMMAND_RESTARTDM:
			g_pLTClient->CPrint("Restart DM here");
			if ( g_pLTCDirectMusicMgr)
			{
				g_pLTCDirectMusicMgr->Term();

				g_pLTCDirectMusicMgr->Init();
				LTRESULT result = g_pLTCDirectMusicMgr->InitLevel("\\Music","music.txt");
				DebugPrintIfFailed(result, "Error in loading direct music level info");
...
void CLTClientShell::UpdateGuiText(int nIndex)
{
	...
	sprintf(sText, "Music\n \n"
		...
		"I     = Initial Play\n"
		"Space = Toggle Pause\n"
		"Left  = Previous Intensity\n"
		"Right = Next Intensity\n\n"
		"Note: Moving into the colored platforms changes the intensity (music).",
		m_nCurIntensity,
		g_sSegments[m_nCurIntensity],
		sState
		);
	m_Gui.SetText(sText);
}
