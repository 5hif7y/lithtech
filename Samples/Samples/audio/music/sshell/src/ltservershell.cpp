...
/*	ObjArray <HOBJECT, 1> pStartPt0;
	g_pLTServer->FindNamedObjects("Trigger0", pStartPt0);
	if (pStartPt0.NumObjects() == 1)
		g_pLTServer->CPrint("Trigger0 found");

	ObjArray <HOBJECT, 1> pStartPt1;
	g_pLTServer->FindNamedObjects("Trigger1", pStartPt1);
	if (pStartPt1.NumObjects() == 1)
		g_pLTServer->CPrint("Trigger1 found");
	
	ObjArray <HOBJECT, 1> pStartPt2;
	g_pLTServer->FindNamedObjects("Trigger2", pStartPt2);
	if (pStartPt2.NumObjects() == 1)
		g_pLTServer->CPrint("Trigger2 found");
	
	ObjArray <HOBJECT, 1> pStartPt3;
	g_pLTServer->FindNamedObjects("Trigger3", pStartPt3);
	if (pStartPt3.NumObjects() == 1)
		g_pLTServer->CPrint("Trigger3 found");
*/
	// Get a list of the startpoint objects with a particular name (should be 1)
