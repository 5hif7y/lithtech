...
namespace
{
	float		g_fTanFovX = 0.0f;
	float		g_fTanFovY = 0.0f;
	CCamera*	g_pCamera = NULL;
	LTIntPt	g_CrosshairPos(320,240);
	uint32   g_crosshairOnARGB = 0xff00ff00;
	uint32   g_crosshairOffARGB = 0xffff0000;
}


//Function to handle filtering of the intersect segment calls needed by the select object / crosshair
inline bool SelectFilterFn(HOBJECT hTest, void *pUserData)
{
	...
	// all of the radios are models and there are not other model in the world
	if ( nObjType == OT_MODEL )
		return true;

// This code doesn't get called but it could be if we had other model we wanted filtered
//	if ( g_pCShell->m_pSoundSampleMgr->IsSampleObject(hTest) )
//		return true;
...
LTRESULT CLTClientShell::OnEngineInitialized(RMode *pMode, LTGUID *pAppGuid)
{
	...
	m_pSoundSampleMgr = new SoundSampleMgr;
	result = m_pSoundSampleMgr->Init();
	DebugPrintIfFailed(result, "Error initializing sound sample manager.\n");

	// Initialize the gui.
	m_pSampleGui = new SampleGui;
	result = m_pSampleGui->Init();
	DebugPrintIfFailed(result, "Error initializing gui.\n");

	// Setup FOV tangents - used for selecting radios
	uint32 nWidth, nHeight;
	GetScreenDims(nWidth, nHeight);
	float fFovX = DEGREES_TO_RADIANS(90.0f);
	g_fTanFovX = tanf( fFovX * 0.5f );
	g_fTanFovY = tanf( ( ((fFovX * (float)nHeight) / (float)nWidth)) * 0.5f );
...
LTRESULT CLTClientShell::InitGame()
{
	...
	// Initialize the selection crosshair
	InitCrossHair();
...
void CLTClientShell::Update()
{
	...
	// Update the sound sample info.
	m_pSampleGui->Update(m_pSoundSampleMgr->GetCurrentSample());
...
LTRESULT CLTClientShell::Render()
{
	...
	// Render the gui.
	result = m_pSampleGui->Render();
	if (LT_OK != result)
	{
		g_pLTClient->DebugOut("Error in gui render\n");
		g_pLTClient->Shutdown();
	}
...
void CLTClientShell::OnCommandOn(int command)
{
    switch (command)
    {
        ...
        case COMMAND_CURSOR_UP:
            {
                m_pSampleGui->MoveCursorUp();
                break;
            }

        case COMMAND_CURSOR_DOWN:
            {
                m_pSampleGui->MoveCursorDown();
                break;
            }

        case COMMAND_CHANGE_ENTRY_UP:
        case COMMAND_CHANGE_ENTRY_DOWN:
            {
                // There are a few commands that should only happen once per button press.
                // The others will be handled in PollInput.
                EGuiCommand GuiCommand = m_pSampleGui->GetCurrentCommand();
                switch (GuiCommand)
                {
                ...
                case GUICOMMAND_PITCH_SHIFT:
                    break; // handle in PollInput instead
                case GUICOMMAND_SAMPLE_NAME:
                case GUICOMMAND_SAMPLE_TYPE:
                case GUICOMMAND_POSITION:
                case GUICOMMAND_LOOPED:
                case GUICOMMAND_PLAY:
                    // Only one of these per button press.
                    m_pSoundSampleMgr->ApplyCommand(GuiCommand, command == COMMAND_CHANGE_ENTRY_UP);
                    break;
                default:
                    g_pLTClient->DebugOut("Unknown gui command!\n");
                    break;
                }
                break;
            }
...
LTRESULT CLTClientShell::PollInput()
{
	...
	if ( g_pLTClient->IsCommandOn(COMMAND_SELECT_OBJECT))
	{
	...
	if	( g_pLTClient->IsCommandOn(COMMAND_CROSSHAIR_LEFT))
	{
		if	(g_CrosshairPos.x	> 10 )
			g_CrosshairPos.x--;
	}

	if	( g_pLTClient->IsCommandOn(COMMAND_CROSSHAIR_RIGHT))
	{
		uint32 nWidth,	nHeight;
		GetScreenDims(nWidth,nHeight );
		if	(g_CrosshairPos.x	< (int) (nWidth -	10)	)
			g_CrosshairPos.x++;
	}

	if	( g_pLTClient->IsCommandOn(COMMAND_CROSSHAIR_DOWN))
	{
		uint32	nWidth, nHeight;
		GetScreenDims(nWidth,nHeight );

		if (g_CrosshairPos.y < (int) (nHeight - 10) )
			g_CrosshairPos.y++;
	}

	if	( g_pLTClient->IsCommandOn(COMMAND_CROSSHAIR_UP))
	{
		if (g_CrosshairPos.y > 10 )
			g_CrosshairPos.y--;
	}

	if	( g_pLTClient->IsCommandOn(COMMAND_CROSSHAIR_CENTER))
	{
		uint32	nWidth, nHeight;
		GetScreenDims(nWidth,nHeight );
		g_CrosshairPos.y = (int) nHeight/2;
		g_CrosshairPos.x = (int) nWidth/2;
	}

	// change entry up
	if (g_pLTClient->IsCommandOn(COMMAND_CHANGE_ENTRY_UP))
	{
        // There are a few commands that should only happen once per button press.
        EGuiCommand GuiCommand = m_pSampleGui->GetCurrentCommand();
        switch (GuiCommand)
        {
            ...
            case GUICOMMAND_VOLUME:
            case GUICOMMAND_PITCH_SHIFT:
                m_pSoundSampleMgr->ApplyCommand(GuiCommand, true);
                break;
            case GUICOMMAND_SAMPLE_NAME:
            case GUICOMMAND_SAMPLE_TYPE:
            case GUICOMMAND_POSITION:
            case GUICOMMAND_LOOPED:
            case GUICOMMAND_PLAY:
                break; // handle in OnCommandOn instead
            default:
                g_pLTClient->DebugOut("Unknown gui command!\n");
                break;
        }
	...
	// change entry down
	if (g_pLTClient->IsCommandOn(COMMAND_CHANGE_ENTRY_DOWN))
	{
        ...
        switch (GuiCommand)
        {
            ...
            case GUICOMMAND_VOLUME:
            case GUICOMMAND_PITCH_SHIFT:
                m_pSoundSampleMgr->ApplyCommand(GuiCommand, false);
                break;
            case GUICOMMAND_SAMPLE_NAME:
            case GUICOMMAND_SAMPLE_TYPE:
            case GUICOMMAND_POSITION:
	...
	// Update the crosshair color and select a radio if requested 
	{
		LTVector vPos = GetWorldFromScreenPos(g_CrosshairPos);
		HOBJECT hObj = IsOverSampleObject(vPos);

		if ( hObj )
		{
			if (g_pLTClient->IsCommandOn(COMMAND_SELECT_OBJECT))
			{
				m_pSoundSampleMgr->SetCurrentSample(hObj);
				m_pSoundSampleMgr->PlayCurrentSample();
			}

			m_nCrossHairARGB = g_crosshairOnARGB;
		}
		else
		{
			m_nCrossHairARGB = g_crosshairOffARGB;
		}
	}
...
void CLTClientShell::OnEngineTerm()
{
	...
	if (m_pSoundSampleMgr != NULL)
	{
		delete m_pSoundSampleMgr;
		m_pSoundSampleMgr = NULL;
	}

	if (m_pSampleGui != NULL)
	{
		delete m_pSampleGui;
		m_pSampleGui = NULL;
	}
...
//-----------------------------------------------------------------------------
//	CLTClientShell::GetWorldFromScreenPos(LTIntPt pos)
//
//-----------------------------------------------------------------------------
#define NEAR_Z 1.0f
#define FAR_Z 10000.0f
LTVector CLTClientShell::GetWorldFromScreenPos(LTIntPt pos)
{
	LTVector vDir;
	...
	float dx = g_fTanFovX * (((float)pos.x) / ((float(nWidth)) *0.5f) -1.0f);
	float dy = g_fTanFovY * (1.0f - ((float)pos.y)/( ((float)nHeight * 0.5f)  ) );

	LTMatrix mCam;
	if(g_pCamera != NULL)
	{
		mCam = m_pCamera->GetCameraTransform();
	}

	LTMatrix mInvCam = mCam.MakeInverseTransform();

	LTVector vWorldPos1(dx * NEAR_Z, dy * NEAR_Z, NEAR_Z);
	LTVector vWorldPos2(dx * FAR_Z, dy * FAR_Z, FAR_Z);

	LTVector vTemp1;
	MatVMul(&vTemp1, &mInvCam, &vWorldPos1);

	LTVector vTemp2;
	MatVMul(&vTemp2, &mInvCam, &vWorldPos2);

	vDir = (vWorldPos2 - vWorldPos1);
	vDir.Normalize();
...
//-----------------------------------------------------------------------------
//	CLTClientShell::GetScreenDims(uint32 &nWidth, uint32 &nHeight)
//
//-----------------------------------------------------------------------------
void CLTClientShell::GetScreenDims(uint32 &nWidth, uint32 &nHeight)
{
...
//-----------------------------------------------------------------------------
//	CLTClientShell::IsOverSampleObject(LTVector &vClick)
//
//-----------------------------------------------------------------------------
HOBJECT CLTClientShell::IsOverSampleObject(LTVector &vClick)
{
	//Casy a ray... hit stuff...

	LTVector vPos, vEndPoint, vForward;
	g_pLTClient->GetObjectPos(m_hPlayer, &vPos);
	LTRotation rRot;
	g_pLTClient->GetObjectRotation(m_hPlayer, &rRot);

	HOBJECT hFilterList[] = {m_hPlayer, LTNULL};
	...
	qInfo.m_From = vPos;
	LTVector vTemp = (rRot.Right() * vClick.x) + (rRot.Up() * vClick.y) + (rRot.Forward() * vClick.z);
	LTVector vEnd = (vTemp * 10000.0f);
	vEnd += vPos;

	qInfo.m_To = vEnd;


//	g_pLTClient->CPrint("Vector: (%f,%f,%f) (%f,%f,%f) \n", vPos.x, vPos.y, vPos.z, vEnd.x, vEnd.y, vEnd.z);

	qInfo.m_Flags = INTERSECT_OBJECTS | IGNORE_NONSOLID;
	qInfo.m_FilterFn = SelectFilterFn;
...
//-----------------------------------------------------------------------------
//	CLTClientShell::InitCrossHair()
//
//-----------------------------------------------------------------------------
void CLTClientShell::InitCrossHair()
{
	memset(&m_Poly[0], 0, sizeof(LT_POLYGT4));
	memset(&m_Poly[1], 0, sizeof(LT_POLYGT4));
...
//-----------------------------------------------------------------------------
//	CLTClientShell::DrawCrossHair()
//
//-----------------------------------------------------------------------------
void CLTClientShell::DrawCrossHair()
{
	uint32 nWidth, nHeight;
	...
	g_pLTCDrawPrim->SetRGBA(&m_Poly[0],m_nCrossHairARGB);
	g_pLTCDrawPrim->SetRGBA(&m_Poly[1],m_nCrossHairARGB);

	float x = (float) g_CrosshairPos.x; 
	float y = (float) g_CrosshairPos.y; 
	float l = 17.0f; 

	g_pLTCDrawPrim->SetXYWH(&m_Poly[0],x-8.0f,y,l,1);
	g_pLTCDrawPrim->SetXYWH(&m_Poly[1],x,y-8.0f,1,l);
	g_pLTCDrawPrim->DrawPrim(&m_Poly[0],2);
