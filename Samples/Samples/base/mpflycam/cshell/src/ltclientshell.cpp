	...
	case MSG_STARTPOINT_POS_ROT:
		{
            ...
            g_pLTClient->SetObjectRotation(m_hPlayer, &m_rPlayerStartRot);
		}
		break;
    ...
    if(m_bInWorld)
    {
        ...
        if( m_bFirstUpdate )
        {
            ...
            if (NULL != m_hPlayer)
            {
                ...
                g_pLTCCommon->SetObjectFlags(m_hPlayer, OFT_Flags, 0, FLAG_VISIBLE);
...
LTRESULT CLTClientShell::PollInput()
{
	...
	if (bMoved)
	{
		//g_pLTCPhysics->MoveObject(m_hPlayer, &vVel, 0);
        g_pLTCPhysics->SetVelocity(m_hPlayer, &vVel);
		LTVector vZero(0.0f, 0.0f, 0.0f);
		g_pLTCPhysics->SetAcceleration(m_hPlayer, &vZero);
	} else {
        LTVector vSlowdownVel;
        g_pLTCPhysics->GetVelocity(m_hPlayer, &vSlowdownVel);
		float fVelMag = vSlowdownVel.Mag();
		if (fVelMag < 0.01f)
		{
			// If the velocity is very small, then set it to zero
			if (fVelMag > 0.0f)
			{
				vSlowdownVel.Init(0.0f, 0.0f, 0.0f);
				g_pLTCPhysics->SetVelocity(m_hPlayer, &vSlowdownVel);
				SendVelocity(vSlowdownVel);
			}
		} else {
			//Coast to a halt
			float fFrameTime = g_pLTClient->GetFrameTime();
			float fUnitsPerSecond = 3750.0f;
			float fSpeedScale = (fUnitsPerSecond/fVelMag) * fFrameTime;
			float fFinal = 1.0f - (fSpeedScale);
			if (fFinal < 0.0f)
				fFinal = 0.001f;
			vSlowdownVel *= fFinal;
			g_pLTCPhysics->SetVelocity(m_hPlayer, &vSlowdownVel);
...
//
//-----------------------------------------------------------------------------
void CLTClientShell::SendVelocity(LTVector &vVel)
{
	ILTMessage_Write *pMessage;
	LTRESULT nResult = g_pLTCCommon->CreateMessage(pMessage);
...
//
//-----------------------------------------------------------------------------
void CLTClientShell::SendVelAndRot(LTVector &vVel, LTRotation &rRot)
{
	ILTMessage_Write *pMessage;
	LTRESULT nResult = g_pLTCCommon->CreateMessage(pMessage);
...
{
    return this->InitGame(EGT_CONNECT, ip);
}
...
{
    return this->InitGame(EGT_NORMAL);
}
