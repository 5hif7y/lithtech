...
void CLTClientShell::OnCommandOn(int command)
{
	switch (command)
	{
	case COMMAND_STACKTRACE:
		{	
			StackTracer stackTracer;
			StackTrace trace;
			stackTracer.DoStackTrace(trace);

			int nTraceSize = (int)trace.size();

			// Print out the Raw Trace.
			for(int i = 0; i < nTraceSize; ++i)
			{
				g_pLTClient->CPrint("Trace [%d] IP: %08I64x Addr: %08I64x EBP: %08I64x", i, trace[i].ip, trace[i].retAddr, trace[i].ebp);
			}	

			// Print out a callstack
			g_pLTClient->CPrint("CallStack:");
			g_pLTClient->CPrint("----------------------------");

			// Load symbols
			// -check the symbol paths as defined in the Autoexec.cfg up to 10 different paths
			for(int i = 0; i < 10; ++i)
			{
				char szSymbolPathVar[64];
				sprintf(szSymbolPathVar, "SymbolPath%d", i);

				HCONSOLEVAR hVar = g_pLTClient->GetConsoleVar(szSymbolPathVar);
				const char* pszVarValue = g_pLTClient->GetVarValueString(hVar);
				if(pszVarValue)
				{
					stackTracer.LoadSymbolsInDir(pszVarValue, "exe,dll,lto,fxd");
				}
			}
			...
			for(int i = 0; i < nTraceSize; ++i)
			{
				...
				stackTracer.GetSymbolByAddr(symDetails, trace[i].retAddr);

				if(symDetails.ErrorCode)
				{
					g_pLTClient->CPrint("  %08I64x()", trace[i].retAddr);
				}
				else
				{
					//strip the file path from the srouce file name
					char szRootName[256];
					StringTokenizer tokenizer(symDetails.Filename, "\\");
					while(tokenizer.hasMoreTokens())
					{
						char* pTok = tokenizer.nextToken();
						if(pTok[0] != '\0')
						{
							strncpy(szRootName, pTok, 255);
						}
					}

					g_pLTClient->CPrint(" %s (%s, line: %d)", symDetails.Name, szRootName, symDetails.LineNumber);
				}
			}
			stackTracer.UnloadSymbols();
