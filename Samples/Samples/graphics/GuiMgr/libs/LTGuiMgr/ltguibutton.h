...
public:
   ...
   LTBOOL  SetFont ( CFontInfo *pFont, uint8 nFontSize);
	...
	LTBOOL	SetText(const wchar_t * pText, LTBOOL bHighlightText = LTFALSE);
	CLTGUIPolyString*	GetString()  {return m_pText;}


	void	SetTexture(HTEXTURE hNormal, HTEXTURE hSelected = LTNULL, HTEXTURE hDisabled = LTNULL, LTBOOL bFreeOld = LTFALSE);

	virtual void	SetCommandHandler(CLTGUICommandHandler *pCommandHandler)	{ m_pCommandHandler = pCommandHandler; }
