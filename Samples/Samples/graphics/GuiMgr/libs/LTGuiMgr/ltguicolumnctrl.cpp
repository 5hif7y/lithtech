...
uint8 CLTGUIColumnCtrl::AddColumn(const wchar_t *pString, uint16 nWidth, LTBOOL bClip)
{
	if ( !pString )
	{
        assert(LTFALSE);
...
CLTGUIPolyString*	CLTGUIColumnCtrl::GetPolyString(uint8 nColumnIndex) const
{
	if (nColumnIndex > kMaxNumColumns || nColumnIndex >= m_columnArray.size())
		return LTNULL;
...
const wchar_t* CLTGUIColumnCtrl::GetString(uint8 nColumnIndex) const
{
	CLTGUIPolyString*	pString = GetPolyString(nColumnIndex);
...
// Set the font
void CLTGUIColumnCtrl::SetFont(CFontInfo *pFont, uint8 nFontSize, LTBOOL bSetForAll)
{
	if (!pFont && !nFontSize)
	{
        return;
