...
public:
    ...
    virtual LTBOOL   Create ( uint32 dwCommandID, uint32 dwHelpID, CFontInfo *pFont, uint8 nFontSize,
	...
	uint8          AddColumn(const wchar_t *pString, uint16 nWidth, LTBOOL bClip = LTFALSE);

	// Gets a string at a specific column index.  This returns a copy (new handle).
	CLTGUITextCtrl*			GetColumn(uint8 nColumnIndex) const;
	CLTGUIPolyString*			GetPolyString(uint8 nColumnIndex) const;
	const wchar_t *			GetString(uint8 nColumnIndex) const;
	...
	// Set the font
    void			SetFont (CFontInfo *pFont, uint8 nFontSize, LTBOOL bSetForAll = LTFALSE);
