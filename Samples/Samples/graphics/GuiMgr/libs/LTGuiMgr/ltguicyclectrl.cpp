...
LTBOOL CLTGUICycleCtrl::Create ( const wchar_t *pText, uint32 nHelpID, 
								CFontInfo *pFont, uint8 nFontSize, uint16 nHeaderWidth, uint8 *pnValue)
{
	...
	if ( pText)
	{
		if (m_pHeaderText)
		{
			m_pHeaderText->SetText(pText);
		}
		else
		{
			m_pHeaderText = debug_new( CLTGUIPolyString );
			m_pHeaderText->SetFont(*m_pFont);
			m_pHeaderText->SetText(pText);
			m_pHeaderText->SetPos(m_pos);
			m_pHeaderText->SetFontHeight(m_nFontSize);
...
void CLTGUICycleCtrl::AddString(const wchar_t *pString)
{
	...
	CLTGUIPolyString * pPolyStr = debug_new( CLTGUIPolyString );

	if (pPolyStr)
	{
		pPolyStr->SetFont(*m_pFont);
		pPolyStr->SetText(pString);
		pPolyStr->SetPos(m_pos);

		pPolyStr->SetFontHeight(m_nFontSize);
...
CLTGUIPolyString* CLTGUICycleCtrl::GetString(uint8 nIndex)
{
...
void CLTGUICycleCtrl::RemoveString(uint8 nIndex)
{
	...
	debug_delete( m_stringArray.at(nIndex) );
...
void CLTGUICycleCtrl::RemoveAll()
{
	...
	{
		debug_delete(*iter); 
//		g_pLTCFontManager->DestroyPolyString(*iter);
		iter++;
	}
	m_stringArray.clear();
	m_nSelIndex=kNoSelection;		
...
// Set the font
LTBOOL CLTGUICycleCtrl::SetFont(CFontInfo *pFont, uint8 nFontSize)
{
	if (!pFont && !nFontSize)
	{
        return LTFALSE;
	...
	if (bApply)
	{
		for (uint8 i = 0; i < m_stringArray.size(); i++)
		{
			...
			m_stringArray[i]->SetFontHeight(m_nFontSize);
		}
		if (m_pHeaderText)
		{
			m_pHeaderText->SetFont(*m_pFont);
...
{ 
	...
	if (m_pHeaderText)
	{
		m_pHeaderText->SetPos((float)m_pos.x,(float)m_pos.y);
	}
	float x = (float)m_pos.x + ((float)m_nHeaderWidth * m_fScale);
	for (uint8 i = 0; i < m_stringArray.size(); i++)
	{
		m_stringArray[i]->SetPos(x,(float)m_pos.y);
	}
}
