...
public:
   ...
   virtual LTBOOL   Create ( ILTClient *pLTClient, uint32 nCommandID, CFontInfo *pFont, uint8 nFontSize, 
								uint16 nMaxLength, CLTGUICommandHandler *pCommandHandler = LTNULL, 
								wchar_t *pStringValue=LTNULL, uint32 nParam1 = 0, uint32 nParam2 = 0 );
   ...
   void            UpdateData(LTBOOL bSaveAndValidate=LTTRUE);
	...
	// Set the font
	LTBOOL			SetFont ( CFontInfo *pFont, uint8 nFontSize );
...
protected:
   ILTClient       *m_pLTClient;
	...
	CFontInfo*	m_pFont;				// The font for this control
	uint8		m_nFontSize;			// The scaled font size to use.
	uint8		m_nBaseFontSize;		// The font size before scaling

	CLTGUIPolyString*		m_pDescription;
	CLTGUIPolyString*		m_pText;

	uint16		m_nMaxLength;			// The maximum length for the string
	uint16		m_nCaretPos;
	wchar_t		*m_pStringValue;		// The pointer that is updated in the UpdateData function
