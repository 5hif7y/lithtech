...
void CLTGUIFrame::Render()
{
	...
	g_pLTCDrawPrim->SetTexture(m_hFrame);
	...
	{
		g_pLTCDrawPrim->DrawPrim(m_Poly,9);
	}

	if (m_nBorderWidth)
	{
		for (int f = 0;f < 4; ++f)
			g_pLTCDrawPrim->SetRGBA(&m_Border[f],m_BorderColor);

		// draw our frames
		g_pLTCDrawPrim->DrawPrim(m_Border,4);
	}
...
void CLTGUIFrame::InitPolies()
{
	...
	if (m_bSimpleStretch)
	{
		...
		g_pLTCDrawPrim->SetRGBA(&m_Poly[0],m_argbColor);
	}
	else
	{
		...
		for (int r = 0; r < 3; r++)
		{
			...
			{
				int i = r*3+c;
				g_pLTCDrawPrim->SetRGBA(&m_Poly[i],0xFFFFFFFF);
...
void CLTGUIFrame::ScalePolies()
{
	...
	if (m_bSimpleStretch)
	{
		...
		g_pLTCDrawPrim->SetXYWH(&m_Poly[0],fx,fy,fw,fh);
	}
	else
	{
		if (!m_hFrame) return;
		g_pLTCTexInterface->GetTextureDims(m_hFrame,tw,th);
		float fw = (float)tw;
		float fh = (float)th;
		...
		for (int r = 0; r < 3; r++)
		{
			...
			{
				int i = r*3+c;
				g_pLTCDrawPrim->SetXY4(&m_Poly[i],x[c],y[r],x[c+1],y[r],x[c+1],y[r+1],x[c],y[r+1]);
			}
		}
	}
	...
	float fh = BorderW;
	g_pLTCDrawPrim->SetXYWH(&m_Border[0],fx,fy,fw,fh);
	...
	g_pLTCDrawPrim->SetXYWH(&m_Border[1],fx,fy,fw,fh);
	...
	fh = BorderW;
	g_pLTCDrawPrim->SetXYWH(&m_Border[2],fx,fy,fw,fh);
	...
	fw = (float)BorderW;
	fh = (float)GetHeight() + BorderW;
	g_pLTCDrawPrim->SetXYWH(&m_Border[3],fx,fy,fw,fh);
