...
// Create the control
LTBOOL CLTGUILargeText::Create (	const wchar_t *pText, CFontInfo *pFont, uint8 nFontSize, LTIntPt ptTextSize )
{
...
void CLTGUILargeText::CalculateLines()
{
	...
	if (m_pString && m_pString->GetStringLength())
	{
...
//		float fYPos = pPolys[nPos].verts[ 0 ].y;
		...
		LTRect2n rRect;
		m_pString->GetCharRect( nPos, rRect );
		float fYPos = (float) m_pos.y + rRect.Top();							//mpPolys[nPos].verts[ 0 ].y;
		...
		while (nPos < m_pString->GetStringLength())
		{

			m_pString->GetCharRect( nPos, rRect );

			//find the top of the char
			float fTestY = (float) m_pos.y + rRect.Top();	//				pPolys[nPos].verts[ 0 ].y;
...
void CLTGUILargeText::Render()
{
	...
	if (m_nFrameWidth)
	{
		...
		for (int f = 0;f < 4; ++f)
			g_pLTCDrawPrim->SetRGBA(&m_Frame[f],GetCurrentColor());
   ...
   uint32 argbColor=GetCurrentColor();
	...
	if (m_nFirstLine <= kMaxNumLines)
	{
		...
		uint16 nLastChar = m_pString->GetStringLength();
...
{ 
	...
	if (m_pString)
	{
		...
		m_pString->SetPos((float)m_pos.x+fi,(float)m_pos.y+line);
	}
...
LTBOOL CLTGUILargeText::OnUp ( )
{
	...
	LTRect2n rRect;
	m_pString->GetCharRect( nPos, rRect );
	float fYPos = (float) m_pos.y + rRect.Top(); // pPolys[nPos].verts[ 0 ].y;

	m_pString->GetCharRect( 0, rRect );
	float fBasePos = (float) m_pos.y + rRect.Top(); //pPolys[0].verts[ 0 ].y;

	float offset = fYPos - fBasePos;

	m_pString->SetPos((float)m_pos.x+fi,(float)m_pos.y-offset);
	CalculateLines();
