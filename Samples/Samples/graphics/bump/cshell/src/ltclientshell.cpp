...
void CLTClientShell::Update()
{
	...
	g_pLTClient->SetObjectPosAndRotation(m_pCamera->GetLight(), &vPos, &rRot);	
...
void CLTClientShell::OnCommandOn(int command)
{
	...
	case COMMAND_CAMERA_LIGHT:
	{
		if ( m_bCameraLight )
		{
			g_pLTCCommon->SetObjectFlags(m_pCamera->GetLight(), OFT_Flags, 0, FLAG_VISIBLE);
		}
		else
		{
			g_pLTCCommon->SetObjectFlags(m_pCamera->GetLight(), OFT_Flags, 1, FLAG_VISIBLE);
		}

		m_bCameraLight=!m_bCameraLight;
