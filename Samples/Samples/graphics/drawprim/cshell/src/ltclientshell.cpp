...
LTRESULT CLTClientShell::OnEngineInitialized(RMode *pMode, LTGUID *pAppGuid)
{
    ...
    // Fill our MeshInfo Array
    strncpy(m_pMeshInfo[0].filename, "USGS\\southseattle.raw", 255);
    m_pMeshInfo[0].x = 317;
    m_pMeshInfo[0].y = 466;

    strncpy(m_pMeshInfo[1].filename, "USGS\\snoqualmie.raw", 255);
    m_pMeshInfo[1].x = 323;
    m_pMeshInfo[1].y = 470;

    strncpy(m_pMeshInfo[2].filename, "USGS\\rainerwest.raw", 255);
    m_pMeshInfo[2].x = 325;
    m_pMeshInfo[2].y = 468;

    // Allocate our dynamic mesh
    m_pDynamicMesh = new DynamicMesh();

    MeshInfo *pMesh = &m_pMeshInfo[m_nCurMesh];
    m_pDynamicMesh->Init(pMesh->filename, pMesh->x, pMesh->y);


    m_Gui.Init("fonts/nolf.ttf", "Nolf", 36);
    m_Gui.SetText(pMesh->filename);
    m_Gui.Bottom();
...
void CLTClientShell::Update()
{
	...
	if( m_bFirstUpdate )
	{
        ...
        // Set our camera for draw prim
        m_pDynamicMesh->SetCamera(m_pCamera->GetCamera());
...
void CLTClientShell::OnCommandOn(int command)
{
	switch (command)
	{
		...
		case COMMAND_PREV_MODEL:
		{
                ...
                if( m_nCurMesh == 0xFF )
                {
                    m_nCurMesh = MAX_DYNAMIC_MESHES - 1;
                }
		...
		{
                m_nCurMesh++;
                if( m_nCurMesh > MAX_DYNAMIC_MESHES - 1 )
                {
                    m_nCurMesh = 0;
                }
