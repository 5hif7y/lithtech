...
#define MOVEMENT_SPEED_DEFAULT 250.0f
#define MOVEMENT_SPEED_RUN_SCALE 4.0f
#define MOVEMENT_SPEED_CRAWL_SCALE 0.25f
float g_fMovementSpeed = MOVEMENT_SPEED_DEFAULT;

uint32 g_nRenderPass = 0;
...
LTRESULT CLTClientShell::OnEngineInitialized(RMode *pMode, LTGUID *pAppGuid)
{
	...
	//
	m_ShaderMgr.SetCamera(m_hCamera);
...
void CLTClientShell::Update()
{
	...
	if( m_bRender )
	{
		m_ShaderMgr.OnUpdate(g_pLTClient->GetFrameTime(), true);
...
LTRESULT CLTClientShell::Render()
{
	...
	// Handle the actual RenderCamera calls...
	m_ShaderMgr.OnRender(fFrameTime);
...
void CLTClientShell::OnCommandOn(int command)
{
	...
	case COMMAND_QUIT:
		...
		break;
	case COMMAND_ID_SHADER_BLACKANDWHITE:
		{
			m_ShaderMgr.SetMode(SHADERMGR_MODE_FULLSCREEN_BLACKANDWHITE);
		}
		break;
	case COMMAND_ID_SHADER_SEPIATONE:
		{
			m_ShaderMgr.SetMode(SHADERMGR_MODE_FULLSCREEN_SEPIATONE);
		}
		break;
	case COMMAND_ID_SHADER_SOBELEDGE:
		{
			m_ShaderMgr.SetMode(SHADERMGR_MODE_FULLSCREEN_SOBELEDGE);
		}
		break;
	case COMMAND_ID_SHADER_INVERSE:
		{
			m_ShaderMgr.SetMode(SHADERMGR_MODE_FULLSCREEN_INVERSE);
		}
		break;
	default:
		break;
...
void CLTClientShell::OnCommandOff(int command)
{
	switch (command)
	{
		...
		case COMMAND_ID_SHADER_INVERSE:
		{
			m_ShaderMgr.SetMode(SHADERMGR_MODE_DEFAULT);
		}
		break;
		default:
		{
...
LTRESULT CLTClientShell::PollInput()
{
	...
	{
		g_fMovementSpeed = MOVEMENT_SPEED_DEFAULT * MOVEMENT_SPEED_RUN_SCALE;
	}
	else
	if(g_pLTClient->IsCommandOn(COMMAND_ID_CRAWL))
	{
		g_fMovementSpeed = MOVEMENT_SPEED_DEFAULT * MOVEMENT_SPEED_CRAWL_SCALE;
	}
	else
	{
		g_fMovementSpeed = MOVEMENT_SPEED_DEFAULT;
	}

	
	if (g_pLTClient->IsCommandOn(COMMAND_MOVE_LEFT))
	{
		vTemp = vRight * fFrameTime * -g_fMovementSpeed;
		vPos += vTemp;
		bMoved = true;
	}
	...
	{
		vTemp = vForward * fFrameTime * g_fMovementSpeed;
		vPos += vTemp;
		bMoved = true;
	}
...
void CLTClientShell::SpecialEffectNotify(HLOCALOBJ hObj, ILTMessage_Read *pMessage)
{
	...
	pMessage->SeekTo(0);
	m_sfxMgr.HandleSFXMsg(hObj, pMessage);
	...
	case MSG_PLAYEROBJ:
		...
		break;
	case MSG_GAMELIGHT_INFO:
		{
			m_hGameLight = hObj;
			m_ShaderMgr.SetGameLight(m_hGameLight);
...
void CLTClientShell::OnEffectShaderSetParams(LTEffectShader *pEffect, CRenderStyle *pRenderStyle, HOBJECT hObj, 
											 const LTShaderDeviceState &ShaderDeviceState)
{
	m_ShaderMgr.OnEffectShaderSetParams(pEffect, pRenderStyle, hObj, ShaderDeviceState);
}
