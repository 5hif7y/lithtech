...
LTRESULT CLTClientShell::OnEngineInitialized(RMode *pMode, LTGUID *pAppGuid)
{
	...
	//Create a font using true type fonts
	LTFontParams fontParams;
	fontParams.Italic = 1;		// true
	m_GuiTTF.Init("fonts\\nolf.TTF", "nolf", 32, &fontParams);
	m_GuiTTF.SetText("This is a true type font!\nEach character has its own width.");
	m_GuiTTF.SetPos(64, 90);

	//Create a font using a bitmap (monospaced)
	m_GuiMBMP.Init("fonts\\CONSOLE_FONT.dtx", 12, 12);
	m_GuiMBMP.SetText("This is a monospace bitmap font!\nAll the characters are the same width.");
	m_GuiMBMP.SetPos(64, 170);

	//Create a font using a bitmap (proportional)
	m_GuiPBMP.Init("fonts\\GE Centurion_14.dtx", 16, 32, false);
	m_GuiPBMP.SetText("This is a proportional bitmap font!\nEach character has its own width.");
	m_GuiPBMP.SetPos(64, 220);


	// Create a font using the ttf wchar font support interface 
	CFontInfo textFont(L"fonts\\msmincho.ttc", 24, LTGB2312_CHARSET);
	m_GuiWide.SetFont ( textFont );

	// this chinese at the end of this string should say "good morning"
	std::wstring	sString (L"This is a Chinese unicode font - \u65E9\u4E0A\u597D\uFF01\u4F60\u597D\uFF01"); 
	
	m_GuiWide.SetText(sString.c_str());
	m_GuiWide.SetPos( 64, 300 );
...
LTRESULT CLTClientShell::Render()
{
	...
	// Render gui (text) created from a .ttf (true type) file
	m_GuiTTF.Render();

	// Render gui (text) created from a bitmap (monospace)
	m_GuiMBMP.Render();

	// Render gui (text) created from a bitmap (proportional)
	m_GuiPBMP.Render();

	// Render wchar text create using a true type find file
	m_GuiWide.Render();

	// Render the main gui object
	m_GuiMgr.Render();
