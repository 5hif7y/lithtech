...
LTRESULT CLTClientShell::OnEngineInitialized(RMode *pMode, LTGUID *pAppGuid)
{
	...
	result = g_ShaderMgr.Init();
	if (ShutdownIfFailed(result, "Error in Init() (ShaderMgr). Shutting down.\n"))
	{
		return result;
	}
...
void CLTClientShell::OnCommandOn(int command)
{
	switch (command)
	{
		...
		case COMMAND_PREV_SHADER:
		{
			g_ShaderMgr.PrevShader();
			g_ModelMgr.ReloadModels();

			m_GuiMgr.UpdateModelGui();
			m_GuiMgr.UpdateShaderGui();

			break;
		}

		case COMMAND_NEXT_SHADER:
		{
			g_ShaderMgr.NextShader();
			g_ModelMgr.ReloadModels();
...
//-----------------------------------------------------------------------------
//  CLTClientShell::OnVertexShaderSetConstants()
//
//-----------------------------------------------------------------------------
void CLTClientShell::OnVertexShaderSetConstants(LTVertexShader *pVertexShader, unsigned iPass,
												CRenderStyle *pRenderStyle, HOBJECT hObj,
												const LTShaderDeviceState &ShaderDeviceState)
{
	g_ShaderMgr.SetConstants(pVertexShader, iPass, pRenderStyle, hObj, ShaderDeviceState);
}



//-----------------------------------------------------------------------------
//  CLTClientShell::OnPixelShaderSetConstants()
//
//-----------------------------------------------------------------------------
void CLTClientShell::OnPixelShaderSetConstants(LTPixelShader *pPixelShader, unsigned iPass,
										   	   CRenderStyle *pRenderStyle, HOBJECT hObj,
										   	   const LTShaderDeviceState &ShaderDeviceState)
{
	g_ShaderMgr.SetConstants(pPixelShader, iPass, pRenderStyle, hObj, ShaderDeviceState);
}
