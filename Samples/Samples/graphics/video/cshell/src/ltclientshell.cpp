...
LTRESULT CLTClientShell::Render()
{
	...
	if ( m_hVideo1 || m_hVideo2 )
	{
		if ( m_hVideo1 )
		{
			g_pLTCVideoMgr->UpdateVideo( m_hVideo1);
	
			if ( g_pLTCVideoMgr->GetVideoStatus( m_hVideo1) == LT_FINISHED )
			{
				g_pLTCVideoMgr->StopVideo(m_hVideo1);
				m_hVideo1 = NULL;
			}
		}
		...
		if ( m_hVideo2 )
		{
			g_pLTCVideoMgr->UpdateVideo( m_hVideo2);
	
			if ( g_pLTCVideoMgr->GetVideoStatus( m_hVideo2) == LT_FINISHED )
			{
				g_pLTCVideoMgr->StopVideo(m_hVideo2);
				m_hVideo2 = NULL;
			}
		}
	}
...
void CLTClientShell::OnCommandOn(int command)
{
	...
	case COMMAND_MOVIE1:
		if ( !m_hVideo1 )
		{
			g_pLTCVideoMgr->StartOnScreenVideo( "Movies\\Jupiter.wmv", PLAYBACK_FULLSCREEN, m_hVideo1 );
		}
		break;
	...
	case COMMAND_MOVIE2:
		if ( !m_hVideo2 )
		{
			g_pLTCVideoMgr->StartOnScreenVideo( "Movies\\clock.avi", PLAYBACK_FULLSCREEN, m_hVideo2 );
		}
		break;
